<div id="sidebar"  class="col-lg-4">
	...
	<?php if(dynamic_sidebar('最新评论')) {?>
		<div class="panel panel-info visible-lg">
			...
			<ul class="list-group list-group-flush">
				<?php
					global $wpdb;
					$sql = "SELECT DISTINCT ID, post_title, post_password, comment_ID, comment_post_ID, comment_author, comment_date_gmt, comment_approved, comment_type,comment_author_url,comment_author_email, comment_content AS com_excerpt FROM $wpdb->comments LEFT OUTER JOIN $wpdb->posts ON ($wpdb->comments.comment_post_ID = $wpdb->posts.ID) WHERE comment_approved = '1' AND comment_type = '' AND comment_author != 'zwwooooo' AND post_password = '' ORDER BY comment_date_gmt DESC LIMIT 5";
					$comments = $wpdb->get_results($sql);
					foreach ($comments as $comment) {
						$output .= "\n<li class=\"list-group-item\">" . get_avatar(get_comment_author_email(), 32) . "<span class=\"comment-arrow\"></span><span class=\"comment-log\"> <a href=\"" . htmlspecialchars(get_comment_link( $comment->comment_ID )) . "\" title=\"on " .$comment->post_title . "\">" . cut_comments_str(strip_tags($comment->com_excerpt),19)."</a></span></li>";
					}
	...
	<div class="panel">
		<ul class="nav nav-pills pills-danger">
		...
		<div class="tab-content">
			...
			<div class="tab-pane fade" id="sidebar-tags"><?php wp_tag_cloud( $args ); ?></div>
			<?php if ( is_home() ) { ?>
